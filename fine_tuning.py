...
class MMCRL(nn.Module):
    def __init__(self, backbone_cls):
        ...
        # Load pretrained weights into the student network
        pretrained_path = config['fine_tune']['pretrained_weights_path']
        state_dict = torch.load(pretrained_path, map_location=config['device'], weights_only=False)
        self.student.load_state_dict(state_dict, strict=False)

        if config['fine_tune']['freeze_backbone']:
...
def train():
    ...
    torch.save(model.student.state_dict(), "fine_tuned_student.pth")
